...
public class ThinkingDataAnalytics {
    ...
    private final static String LIB_VERSION = "1.7.0";
    ...
    public static class BatchConsumer implements Consumer {
        ...
        private final List<Map<String, Object>> messageChannel;
        ...
        public BatchConsumer(String serverUrl, String appId) throws URISyntaxException {
            this(serverUrl, appId, 20, null, false, 0);
        }
        ...
        public BatchConsumer(String serverUrl, String appId, Config config) throws URISyntaxException {
            this(serverUrl, appId, Math.min(config.batchSize, MAX_CACHE_SIZE), config.timeout, config.autoFlush, config.interval, config.compress);
        }
         ...
         */
        public BatchConsumer(String serverUrl, String appId, int batchSize, Integer timeout, boolean autoFlush, int interval) throws URISyntaxException {
            this(serverUrl, appId, Math.min(batchSize, MAX_CACHE_SIZE), timeout, autoFlush, interval, "gzip");
        }
         ...
         */
        public BatchConsumer(String serverUrl, String appId, int batchSize, Integer timeout, boolean autoFlush, int interval, String compress) throws URISyntaxException {
        ...
        public void add(Map<String, Object> message) {
            synchronized (messageLock) {
                ...
                if (messageChannel.size() >= batchSize) {
                    flush();
                }
            }
        }
        ...
        public void flush() {
            synchronized (messageLock) {
                ...
                try {
                    ...
                    if (messageChannel.size() > batchSize) {
                        messageList = messageChannel.subList(0, batchSize);
                    } else if (messageChannel.size() == 0) {
                        return;
                    } else {
                        messageList = messageChannel;
                    }
                    ...
                    if (deleteData) {
                        messageChannel.subList(0, Math.min(messageChannel.size(), batchSize)).clear();
                    } else {
                        if (messageChannel.size() > MAX_CACHE_SIZE) {
                            messageChannel.subList(0, messageChannel.size() - MAX_CACHE_SIZE).clear();
                        }
                    }
                }
            }
